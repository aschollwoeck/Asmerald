...
## Example
Simple query using Germanium and Dapper:
```C#
// Build query with Germanium
...
1. Use binary (.exe) of Germanium.Generate to create classes from database
2. Include Germanium library in your project

### Prerequisite
Germanium builds upon an existing database scheme and makes the output available for databases providers to query from the database.
...
## Benchmark
Benchmarking a simple SQL query which benefits Germanium
```SQL
SELECT Id, Name, Form as 'test' FROM cards WHERE id > @idGr ORDER BY Id LIMIT @limit
```
and translated to code with Germanium
```C#
dslCtxt
...
| Germanium | 191.4 us | 2.28 us | 2.02 us |  1.07 |    0.02 |

Germanium adds about 7% overhead.
