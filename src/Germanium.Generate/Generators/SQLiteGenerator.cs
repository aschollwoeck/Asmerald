...
namespace Germanium.Generate.Generators
{
    ...
    public class SQLiteGenerator : IGenerator
    {
        ...
        private Type GetColumnType(string columnDataType)
        {
            ...
            if (columnDataType.Contains("INT")) return typeof(Germanium.Columns.IntegerColumn);
            if (columnDataType.ContainsOneOf(new[] { "CHAR", "CLOB", "TEXT" })) return typeof(Germanium.Columns.StringColumn);
            if (columnDataType.Contains("BLOB")) return typeof(Germanium.Columns.BlobColumn);
            if (columnDataType.ContainsOneOf(new[] { "REAL", "FLOA", "DOUB" })) return typeof(Germanium.Columns.DecimalColumn);

            return typeof(Germanium.Columns.DecimalColumn);
        }
    }
}
