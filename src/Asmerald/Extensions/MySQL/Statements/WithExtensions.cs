...
namespace Asmerald.MySQL
{
    ...
    {
        public static WithTableStatement<T> With<T>(this MySQLDSLContext stmt, params ISelectExpression[] selectColumns)
            where T : ITable, new()
        {
            return new WithTableStatement<T>(stmt.QueryBuilder, selectColumns);
        }
        public static WithStatement With(this MySQLDSLContext stmt)
        {
