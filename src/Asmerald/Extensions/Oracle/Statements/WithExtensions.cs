...
namespace Asmerald.Oracle
{
    ...
    {
        public static WithTableStatement<T> With<T>(this OracleDSLContext stmt, params ISelectExpression[] selectColumns)
            where T : ITable, new()
        {
            return new WithTableStatement<T>(stmt.QueryBuilder, selectColumns);
        }
        public static WithStatement With(this OracleDSLContext stmt)
        {
