...
namespace Asmerald
{
    internal class MSSQLTransformer : ISqlTransformer
    {
        public Dictionary<string, object> Transform(IStatement statement, StringBuilder builder, ref int paraCount)
        {
            Dictionary<string, object> parameters = new Dictionary<string, object>();

            if (statement is ExecStatement execStatement)
            {
                builder.Append($"CALL {execStatement.Sp.Name()}(");
                bool bPara = false;
                foreach (var (k, v) in execStatement.Sp.GetParameters())
                {
                    if (bPara)
                    {
                        builder.Append(", ");
