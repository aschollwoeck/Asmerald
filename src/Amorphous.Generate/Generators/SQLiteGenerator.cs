...
namespace Amorphous.Generate.Generators
{
    /// <summary>
    ...
    public class SQLiteGenerator : IGenerator
    {
        ...
        private Type GetColumnType(string columnDataType)
        {
            ...
            if (columnDataType.Contains("INT")) return typeof(Amorphous.Columns.IntegerColumn);
            if (columnDataType.ContainsOneOf(new[] { "CHAR", "CLOB", "TEXT" })) return typeof(Amorphous.Columns.StringColumn);
            if (columnDataType.Contains("BLOB")) return typeof(Amorphous.Columns.BlobColumn);
            if (columnDataType.ContainsOneOf(new[] { "REAL", "FLOA", "DOUB" })) return typeof(Amorphous.Columns.DecimalColumn);

            return typeof(Amorphous.Columns.DecimalColumn);
        }
    }
}
