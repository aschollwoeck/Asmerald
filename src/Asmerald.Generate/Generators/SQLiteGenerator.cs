...
namespace Asmerald.Generate.Generators
{
    ...
    public class SQLiteGenerator : IGenerator
    {
        ...
        private Type GetColumnType(string columnDataType)
        {
            ...
            if (columnDataType.Contains("INT")) return typeof(Asmerald.Columns.IntegerColumn);
            if (columnDataType.ContainsOneOf(new[] { "CHAR", "CLOB", "TEXT" })) return typeof(Asmerald.Columns.StringColumn);
            if (columnDataType.Contains("BLOB")) return typeof(Asmerald.Columns.BlobColumn);
            if (columnDataType.ContainsOneOf(new[] { "REAL", "FLOA", "DOUB" })) return typeof(Asmerald.Columns.DecimalColumn);

            return typeof(Asmerald.Columns.DecimalColumn);
        }
    }
}
